...
async function readCampaignContext(campaignId: string) {
...
async function readIdeaContext(ideaId: string, campaignContextId?: string) {
  const [row] = await db
    .select({
  ...
  if (!campaignContextId || campaignContextId === row.campaignId) {
    return row;
  }

  const context = await readCampaignContext(campaignContextId);

  return {
    ...row,
    campaignId: context.campaignId,
    campaignPrompt: context.campaignPrompt,
...
async function assertNoClarificationBatch(ideaId: string, campaignContextId: string) {
  const [existing] = await db
    .select({ id: clarificationRequests.id })
    .from(clarificationRequests)
    .where(
      and(
        eq(clarificationRequests.ideaId, ideaId),
        eq(clarificationRequests.campaignContextId, campaignContextId),
  ...
  if (existing) {
    throw new Error("Clarification batch already exists for this idea in this campaign context.");
  }
}
...
export async function requestEmployeeClarification(
  ...
  const idea = await readIdeaContext(ideaId, options.campaignContextId);
  await assertCanTriggerClarification(session, idea.campaignId);
  await assertNoClarificationBatch(idea.ideaId, idea.campaignId);
  ...
  await db.transaction(async (tx) => {
    await tx.insert(clarificationRequests).values({
      campaignContextId: idea.campaignId,
      createdAt: now,
      emailStatus: "pending",
      expiresAt,
      id: requestId,
    ...
    if (options.markIdeaWorkflowState !== false) {
...
export async function runClarificationReminderJob(options: {
  ...
  const rows = await db
    .select({
      campaignTitle: campaigns.publicTitle,
      campaignContextId: clarificationRequests.campaignContextId,
  ...
  for (const row of rows) {
    ...
    const link = buildClarificationLink(options.appBaseUrl ?? getAppBaseUrl(), token);
    const contextCampaign = row.campaignContextId ? await readCampaignContext(row.campaignContextId) : null;
    const campaignTitle = contextCampaign?.campaignTitle ?? (row.previewTitle || row.campaignTitle);
...
async function readRequestByToken(token: string) {
  ...
  if (!row?.campaignContextId) {
    return row ?? null;
  }

  const context = await readCampaignContext(row.campaignContextId);
  return {
    ...row,
    campaignPrompt: context.campaignPrompt,
    campaignTitle: context.campaignTitle
...
export async function getAccessibleClarificationReviewView(session: DemoSession) {
  ...
  const accessible = [];
  for (const row of rows) {
    const campaignContextId = row.campaignContextId ?? row.campaignId;
    if (!(await canContributeToCampaign(session, campaignContextId))) {
      continue;
    }
    const context = row.campaignContextId ? await readCampaignContext(row.campaignContextId) : null;
    accessible.push({
      ...row,
      campaignTitle: context?.campaignTitle ?? row.campaignTitle
    });
  }
...
export async function getAccessibleClarificationTriggerIdeas(session: DemoSession) {
  ...
  const uniqueIdeaRows = Array.from(
    new Map(ideaRows.map((idea) => [`${idea.campaignId}:${idea.ideaId}`, idea])).values()
  );
  const ideaIds = uniqueIdeaRows.map((idea) => idea.ideaId);
  if (ideaIds.length === 0) {
    return [];
  }
  ...
  return uniqueIdeaRows.map((idea) => ({
    ...idea,
